...
Note:
    ...
    - Added timing measurements for pathfinding performance analysis
'''
...
class HexGrid:
    ...
    def find_path(self, start_offset, goal_offset):
        """Find path using A* with cube coordinates and timing."""
        # Start timing
        ...
        if not self.is_valid_offset(*start_offset) or not self.is_valid_offset(*goal_offset):
            ...
            self.timer.add_measurement(execution_time, 0, self.nodes_explored, self.rows * self.cols)
            return []
        
        if start_cube == goal_cube:
            end_time = time.perf_counter()
            execution_time = end_time - start_time
            self.timer.add_measurement(execution_time, 1, self.nodes_explored, self.rows * self.cols)
            return [start_offset]
    ...
    def benchmark_pathfinding(self, num_tests=100):
        """Run benchmark tests with random start/goal positions."""
        print(f"Running {num_tests} pathfinding benchmark tests...")
        
        valid_positions = []
        for row in range(self.rows):
            for col in range(self.cols):
                if self.grid[row][col] == 0:  # Only walkable cells
                    valid_positions.append((row, col))
        
        if len(valid_positions) < 2:
            print("Not enough valid positions for benchmarking")
            return
                
        for i in range(num_tests):
            # Pick random start and goal positions
...
class HexGameVisualization:
    def __init__(self, width=1400, height=850):
        ...
        pygame.display.set_caption("Hexagonal Grid Pathfinding with Performance Timing")
    ...
    def draw_ui(self):
        """Draw user interface elements with timing information."""
        ui_texts = [
            f"Mode: {self.mode}",
            "Left Click: Set start/goal/toggle obstacles",
            "Right Click: Clear cell",
            "SPACE: Find path",
            "B: Run benchmark (100 tests)",
            "R: Reset",
        ...
        # Show path info
        if self.path:
            path_info = f"Current path: {len(self.path)} steps"
            ...
            text_surface = self.font.render(path_info, True, YELLOW)
    ...
    def run_benchmark(self):
        ...
        self.pathfinder = HexGrid(self.grid)
        self.pathfinder.benchmark_pathfinding(100)
